...
interface WebServiceDependencies {
  database?: any
  repository?: any
  telegramBot?: any
}
...
/**
 * Сервис веб-сервера с админ панелью для Telegram бота
 * ПРИМЕЧАНИЕ: Для активации установите Fastify:
 * npm install fastify @fastify/cors @fastify/static
 */
export class WebServerService implements IService {
  private config: AppConfig
  private logger: Logger
  private dependencies: WebServiceDependencies
  ...
  constructor(config: AppConfig, logger: Logger, dependencies: WebServiceDependencies = {}) {
    this.config = config
    this.logger = logger
  ...
  async initialize(): Promise<void> {
    this.logger.i("🌐 Initializing web server...")
    ...
    try {
      ...
      try {
        ...
        this.logger.i("✅ Fastify available - web server can be enabled")
      } catch (error) {
        this.logger.w("⚠️ Fastify not available. Web server disabled.")
      ...
      this.logger.i("✅ Web server service initialized")
    }
    catch (error) {
      this.logger.e("❌ Failed to initialize web server:", error)
  ...
  async start(): Promise<void> {
    this.logger.i("🚀 Starting web server...")
    
    if (!this.hasFastify) {
      this.logger.w("🚫 Web server not available - Fastify not installed")
    ...
    try {
      ...
      this.isRunning = true
      this.logger.i(`✅ Web server started on http://${this.config.WEB_HOST}:${this.config.WEB_PORT}`)
      this.logger.i(`📱 Telegram WebApp URL: http://${this.config.WEB_HOST}:${this.config.WEB_PORT}/admin`)
    }
    catch (error) {
      this.logger.e("❌ Failed to start web server:", error)
  ...
  async stop(): Promise<void> {
    if (this.isRunning) {
      this.logger.i("🛑 Stopping web server...")
      this.isRunning = false
      this.logger.i("✅ Web server stopped")
    }
  }
  ...
  async dispose(): Promise<void> {
    this.logger.i("🗑️ Disposing web server service...")
    await this.stop()
    this.logger.i("✅ Web server service disposed")
  }
