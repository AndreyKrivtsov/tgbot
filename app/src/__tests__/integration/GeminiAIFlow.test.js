...
describe('Gemini AI Integration Flow', () => {
  ...
  test('должен создавать экземпляр и генерировать ответ', async () => {
  ...
  test('должен обрабатывать ошибку при пустом API ключе', async () => {
    await expect(
      geminiAdapter.generateContent('', 'Привет!')
    ).rejects.toThrow('Gemini API key is required');
  });

  test('должен тестировать подключение', async () => {
