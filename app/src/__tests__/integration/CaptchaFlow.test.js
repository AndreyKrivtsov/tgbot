...
describe('Captcha Integration Flow', () => {
  ...
  test('должен генерировать капчу', () => {
  ...
  test('должен принимать правильный ответ', () => {
    const captchaService = new MockCaptchaService();
    const captcha = captchaService.generateCaptcha(123);
  ...
  test('должен отклонять неправильный ответ', () => {
    const captchaService = new MockCaptchaService();
    const captcha = captchaService.generateCaptcha(123);
  ...
  test('должен кикать после превышения попыток', () => {
    const captchaService = new MockCaptchaService();
    const captcha = captchaService.generateCaptcha(123);
